...
namespace BGC.Audio
{
    ...
    public static class WaveEncoding
    {
            ...
            out int channels)
        {
            ...
            Debug.Assert(header == "RIFF", $"Unexpected initial chunkID: {header}");
            ...
            Debug.Assert(waveID == "WAVE", $"Unexpected WaveID: {waveID}");
        ...
        private static bool ReadFMT(
            ...
            out Format format)
        {
            Debug.Assert(buffer.Length >= 16, $"Unexpected fmt ChunkSize: {buffer.Length}");
            ...
            Debug.Assert(channels == 1 || channels == 2, $"Unsupported Channels: {channels}");
            ...
            Debug.Assert(bytesPerSample == 2 || bytesPerSample == 4, $"Unexpected ByteRate: {byteRate}");

            //Block Align - 2 bytes
            short blockAlign = BitConverter.ToInt16(buffer, 12);
            Debug.Assert(blockAlign == channels * bytesPerSample, $"Unexpected BlockAlign: {blockAlign}");

            //Bits Per Sample - 2 bytes
            short bps = BitConverter.ToInt16(buffer, 14);
            Debug.Assert(bps == 8 * bytesPerSample, $"Unexpected BitsPerSample: {bps}");
